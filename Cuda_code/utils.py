...
def compute_region_score(similarities, bbox):
    """Area-normalized region score as defined in HIRES paper.

    Score_j = sum(Sim(p_i, v_q)) / Area(B_j)

    Args:
        similarities: list of cosine similarity values for patches in the component
        bbox: (x_min, y_min, x_max, y_max) bounding box in patch coordinates
    Returns:
        float: area-normalized score (higher = better)
    """
    x_min, y_min, x_max, y_max = bbox
    area = (x_max - x_min + 1) * (y_max - y_min + 1)  # in patch units
    total_sim = sum(similarities)
    return total_sim / area if area > 0 else 0.0
